const renderOtherUserProfile = async (params) => {
    ...
    try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            console.error('Yanıt JSON değil:', text);
        ...
        if (!friendListResponse.ok) {
            const text = await friendListResponse.text();
            console.error('Yanıt JSON değil:', text);
            throw new Error('Yanıt JSON değil');
        }
        ...
        const loadMatchHistory = async () => {
            const matchHistoryResponse = await authFetch(`/api/user/${userId}/games`, {
                method: 'GET',
            });
